...
"""
    重要：⚠ 文件的读取机制：
        程序在执行的时候，有它们自己的文件路径。
        在哪里执行代码，初始路径就在哪里。
            假如在main.py中要调用read_file()方法，则应该写：print(read_file('./class07_file/hcc.txt'))
...
# 封装文件的操作函数
def read_file(f):
    with open(file=f, mode='r', encoding='utf-8') as file1:
        content = file1.read()
    return content

# 绝对路径函数
# def read_file():
#     with open(file='D:\Project\Python\hcc_python_class\class07_file\paper1.txt', mode='r', encoding='utf-8') as file1:
#         content = file1.read()
#     return content
